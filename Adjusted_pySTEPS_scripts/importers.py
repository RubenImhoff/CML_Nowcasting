...
** Adjusted by Ruben Imhoff to allow for the import of GeoTiff in the KNMI
data format. To use this file, place it in your pysteps distrubition as
"importers.py" in the folder .\pysteps\io\importers.py.
...
def import_rainlink_cml(filename, **kwargs):
    """Import an interpolated precipitation field from commercial microwave
    links (CML) in GeoTIFF-format. 

    Parameters
    ----------

    filename : str
        Name of the interpolated CML-file to import (a GeoTIFF-file).

    Other Parameters
    ----------------

    qty : {'RATE', 'ACRR', 'DBZH'}
        The quantity to read from the file. 'RATE' = rainfall rate (mm/h), 
        'ACRR'=hourly rainfall accumulation (mm) and 'DBZH'=max-reflectivity
        (dBZ). The default value is 'ACRR'.
    ...
    Every data type has a slightly different naming convention, depending on
    the source area (e.g. country) and interpolation steps. The standard setup 
    is based on the interpolated rainfall intensities in the Netherlands for the
    summer of 2012. That data is interpolated to the standard KNMI radar grid 
    with 1 km2 spatial and (in this case) 15 min temporal resolution.
    Most CML data is commercially owned and therefore not often distributed. A
    raw CML dataset for the Netherlands during summer (JJA) 2012 is freely 
    available at: https://data.4tu.nl/repository/uuid:323587ea-82b7-4cff-b123-c660424345e5.
    The raw link data can be converted to rainfall intensities via the RAINLINK
    tool (https://github.com/overeem11/RAINLINK) which returns the data in
    1D arrays in a .dat-format with cells corresponding to locations on the land 
    surface. To obtain a GeoTIFF (or e.g. hdf5), the data has to be set on a
    rectangular 2D grid before using this tool.
        
    Note that both the data and standard setup + calibration of RAINLINK are based
    on the Netherlands. 
    """
    ...
    if not GDAL_IMPORTED:
        raise MissingOptionalDependency(
            "gdal package is required to import "
            "CML rainfall intensity composite in GeoTIFF format "
            "but it is not installed"
        )
    ...
    # The time step. Generally, the 5 min data is used, but also hourly, daily
    # and monthly accumulations are present.
    accutime = kwargs.get("accutime", 15.0)
    # The pixel size. Recommended is to use KNMI datasets with 1 km grid cell size.
    # 1.0 or 2.4 km datasets are available - give pixelsize in meters
    pixelsize = kwargs.get("pixelsize", 1000.0)
    ...
    precip_intermediate = f.GetRasterBand(1).ReadAsArray().astype(float)
    
    # In case R is a rainfall intensity (RATE), the no data value is 65535.
    if qty == "RATE":
        precip = np.where(precip_intermediate == 65535,
                          np.NaN,
                          precip_intermediate)
    
    # In case R is a rainfall accumulation (ACRR), the no data value is 65535.
    if qty == "ACRR":
        precip = np.where(precip_intermediate == 65535,
                          np.NaN,
                          precip_intermediate)
    ...
    # Get the projection
    proj4str = '+proj=stere +lat_0=90 +lon_0=0.0 +lat_ts=60.0 +a=6378.137 +b=6356.752 +x_0=0 +y_0=0'
